...
static void LEDMAT_voidDisableCols(void)
{
    ...
    for (col = 0; col < 8; col++)
    {
    	GPIO_u8SetPinValue(COLUM_PORT,cols[col], HIGH);
    }
}
...
static void LEDMAT_voidSetRowVlaues(u8 Copy_u8Frame)
{
    ...
    for (row = 0; row < 8; row++)
    {
    	GPIO_u8SetPinValue(ROW_PORT,rows[row], GET_BIT(Copy_u8Frame, row));
    }
}



void LEDM_voidDisplay(u8 *Copy_u8Frame)
{
    u8 col;
    u8 cols[] = {COLUM0, COLUM1, COLUM2, COLUM3, COLUM4, COLUM5, COLUM6, COLUM7};
    ...
    for (col = 0; col < 8; col++)
    {
        ...
        GPIO_u8SetPinValue(COLUM_PORT,cols[col], LOW);
        
        // Delay 1042 us
//        STK_voidSetBusyWait(1042); // 2.5ms
    }
}
